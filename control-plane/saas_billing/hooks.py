...
def post_init_hook(env):
    """Corrects saas.instance's stored margin fields on a database that
    had EXISTING instances before this install/upgrade.

    monthly_cost/monthly_revenue/monthly_margin/margin_pct/is_profitable
    are store=True (needed so the Profitability dashboard can sort/group
    by them — non-stored fields can't be used in SQL ORDER BY). Storing
    them means Odoo eagerly recomputes and backfills every existing row
    during saas_core's OWN module init — which runs BEFORE this addon
    (saas_billing, which owns saas.plan.price/yearly_price and
    saas.instance.support_plan_id) has loaded. saas_core's
    _instance_monthly_revenue() defends against the resulting missing-
    field AttributeError by returning 0 in that window (see its
    docstring), which leaves every existing instance's stored revenue/
    margin temporarily wrong (looks like 100% loss) right after the
    upgrade — until something else happens to change one of
    _compute_margin's tracked @api.depends fields.

    This hook runs at the END of saas_billing's own installation, once
    price/yearly_price/support_plan_id genuinely exist on the merged
    model — so a plain, honest recompute here (not a hand-rolled SQL
    UPDATE) gets every existing instance's stored margin figures correct
    immediately, without waiting on an unrelated future write.

    A fresh install (or a database where saas_core has never been
    installed before this cutover) has zero existing saas.instance rows
    to backfill, so this is a no-op there — cheap to always run.
    """
    instances = env['saas.instance'].sudo().with_context(active_test=False).search([])
    if not instances:
        return
    instances._compute_margin()
    instances.flush_recordset([
        'margin_currency_id', 'monthly_cost', 'monthly_revenue',
        'monthly_margin', 'margin_pct', 'is_profitable',
    ])
    _logger.info(
        "saas_billing post_init_hook: recomputed and stored correct "
        "margin figures for %d existing saas.instance record(s).",
        len(instances))
